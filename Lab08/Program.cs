...
namespace Lab08
{
    class Program
    {
        ...
        static void DataSourceLamda()
        {
            var queryAllCustomers = context.clientes.Select(clientes => clientes);
            foreach (var item in queryAllCustomers)
            {
                Console.WriteLine(item.NombreCompañia);
            }
        ...
        {
            var queryLondonCustomers3 = context.clientes.Where(x => x.Ciudad == "London").OrderByDescending(x => x.NombreCompañia);
        ...
        {
            var queryCustomerByCity =
                context.clientes.GroupBy(x => x.Ciudad);
